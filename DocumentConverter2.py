...
class DocumentConverter:
    ...
    def convert(self, inputFile, outputFile, pageRange):
        ...
        for index in range(sheets.getCount()):
            ...
            print("name:%s, col count:%d" % (sheet.getName(), columns.getCount()))
            #for i in range(columns.getCount()):
            for i in range(100):
                column = columns.getByIndex(i)
                #print("width:%d" % column.Width)
            #print("%s" % sheet)
        '''
        ...
        try:
            myprop = {"PageRange": pageRange}
            storeProperties["FilterData"] = uno.Any("[]com.sun.star.beans.PropertyValue", self._toProperties(myprop))
            prop = self._toProperties(storeProperties)
            document.storeToURL(outputUrl, prop)
        finally:
            document.close(True)
    ...
    def _overridePageStyleProperties(self, document, family):
        if family in PAGE_STYLE_OVERRIDE_PROPERTIES:
            ...
            for styleName in pageStyles.getElementNames():
                ...
                size = pageStyle.getPropertyValue("Size")
                print("page:%s,width:%d,height:%d" % (styleName, size.Width, size.Height))
...
if __name__ == "__main__":
    ...
    if len(argv) < 3:
        print("USAGE: python %s <input-file> <output-file> <range>" % argv[0])
        exit(255)
    ...
    try:
        ...
        converter.convert(argv[1], argv[2], pageRange)
